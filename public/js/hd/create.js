  ...
  $('#selectblock').on('change', function(e) {
    ...
    if (id_block == 'empty') {
      ...
      $.ajax({
        ...
        $('#selectubicacion').append('<option value="' + data.block.id + '">' + data.block.estacion + '</option>').selectpicker('refresh');

        $.each(data.desvios, function(index, desvioObj) {
          if (index == 0) { // Esto no funciona
            $('#selectubicacion').append('<optgroup label="Desvíos">').selectpicker('refresh');
          };
          $('#selectubicacion').append('<option value="' + desvioObj.id + '">' + desvioObj.nombre + '</option>').selectpicker('refresh');
          if (index == data.desvios.length - 1) { // Esto no funciona
            $('#selectubicacion').append('</optgroup>').selectpicker('refresh');
          };
        });

        $.each(data.desviadores, function(index, desviadoresObj) {
          $('#selectubicacion').append('<option value="' + desviadoresObj.id + '">' + desviadoresObj.nombre + '</option>').selectpicker('refresh');
        });
