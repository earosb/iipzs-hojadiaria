...
class ReporteController extends \BaseController
{
    ...
    public function postFormMayor()
    {
        ...
        // Nombre del archivo
        $filename = 'Form 2-3-4 ' . $sector->nombre . ' [' . $year . '][' . $desde . '-' . $hasta . ']';
        ...
        $this->createGenerador($sector, $year, $desde, $hasta, 'mayor', $filename);
    ...
    public function createGenerador($sector, $year, $desde, $hasta, $tipoTrabajo, $path)
    {
        $blocks = $sector->blocks;
        ...
        $partidas = Trabajo::where('trabajo.es_oficial', '=', '1', 'and')
            ...
            $desdeQuery = date('Y-m-01', strtotime($year . '-' . $desde . '-01'));
            $hastaQuery = date('Y-m-t', strtotime($year . '-' . $hasta . '-01'));

            foreach ($partidas as $trabajo) {
                Excel::create($trabajo->nombre , function ($excel) use ($blocks, $trabajo, $desdeQuery, $hastaQuery){
                    foreach ($blocks as $block) {

                        $trabajos = DB::table('hoja_diaria')
                            ->select(array('fecha', 'block.estacion', 'detalle_hoja_diaria.km_inicio', 'detalle_hoja_diaria.km_termino', 'trabajo.nombre', 'trabajo.unidad', 'cantidad'))
                            ->join('detalle_hoja_diaria', 'hoja_diaria.id', '=', 'detalle_hoja_diaria.hoja_diaria_id')
                            ->join('trabajo', 'detalle_hoja_diaria.trabajo_id', '=', 'trabajo.id')
                            ->join('block', 'detalle_hoja_diaria.block_id', '=', 'block.id')
                        ...
                        if ($trabajos->count()){
                            $excel->sheet($block->estacion, function ($sheet) use ($trabajos){
                                $sheet->loadView('reporte.generador')->with("trabajos", $trabajos);
                            });
                        }
                    }
    ...
    public function createZip($path){
        $source_dir = public_path('excel/'.$path);
        $zip_file = public_path('excel/'.$path.'.zip');
